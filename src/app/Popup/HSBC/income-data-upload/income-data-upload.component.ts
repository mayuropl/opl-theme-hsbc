...
import { ExcelDownloadService } from '../../../CommoUtils/common-services/excel-download.service';
import { SharedService } from '../../../services/SharedService';
import { UniquePopupComponent } from '../unique-popup/unique-popup.component';
...
import {CibilDownloadUrls, Constants} from '../../../CommoUtils/constants';

@Component({
    selector: 'app-income-data-upload',
    templateUrl: './income-data-upload.component.html',
    styleUrl: './income-data-upload.component.scss'
})
export class IncomeDataUploadComponent implements OnInit {
    ...
    constructor(public dialog: MatDialog,
        ...
        private downloadExcelService: ExcelDownloadService,
        private sharedService: SharedService) {

        this.sharedService.getIncomeExcelStatusSubjectChangeEvent().subscribe((message) => {
    ...
    ngOnInit(): void {
      ...
      if (!this.pageData || this.pageData === 'undefined') {
        this.pageData = this.commonService.getPageData(this.consValue.pageMaster.BULK_UPLOAD, this.consValue.pageMaster.INCOME_DATA_UPLOAD);
      }
        resetGlobalHeaders();
    ...
    fetchBulkUploadHistory(page?, onPageChangeFlag?: boolean, approvalStatus?: string): void {
        ...
        data.pageNumber = this.page - 1;

        this.msmeService.incomeDashboardUploadHistory(data).subscribe((res: any) => {
            console.log('res: ', res);
            if (res && res.status == 200) {
                ...
                this.bulkUploadHistory = res?.data?.listData;
    ...
    createBulkUploadForm(bulkUploadDetails) {
        this.uploadDetailsForm = this.formBuilder.group({
            option: new UntypedFormControl(this.incomeMap['1'], [Validators.required]),
    ...
    submit() {
        ...
        const type = (this.uploadDetailsForm.get('option')?.value);
        // if (type === 2) {
        ...
        this.msmeService.incomeDashboardUpload(formData, type).subscribe({
            next: res => {
    ...
    getData(type?, customer?) {
        ...
        createMasterJson[type] = true;
        createMasterJson.mstId = customer;
        this.commonService.successSnackBar('Your file is being prepared. The download will begin shortly…');

        this.msmeService.incomeDashboardFileDownload(createMasterJson).subscribe((res: any) => {
            console.log('res: ', res);
            if (res && res.status == 200) {
                this.downloadCSVFromBase64(res?.data?.fileBytes, res?.data?.fileName);
            } else {
                // console.error;
                this.commonService.warningSnackBar(res.message);
            }
    ...
    incomeMap: any = IncomeUploadMap;

    protected readonly CibilDownloadUrls = CibilDownloadUrls;
}

export const IncomeUploadMap: { [key: string]: string } = {
    '1': 'MONTHLY_INCOME',
    '2': 'MASTER_GROUP_CUSTOMER',
    '3': 'GID_GC_REGION',
    '4': 'NON_GID_GC_REGION'
};
