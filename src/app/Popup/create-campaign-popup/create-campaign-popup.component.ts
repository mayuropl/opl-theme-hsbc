...
@Component({
  ...
  styleUrl: './create-campaign-popup.component.scss',
  animations: [
    trigger('fadeInOut', [
      transition(':enter', [
...
export class CreateCampaignPopupComponent {
  hidden: boolean = true;
  constructor(
    private router: Router, private dialogRef: MatDialogRef<CreateCampaignPopupComponent>, @Inject(MAT_DIALOG_DATA) public data: any,
    ...
    this.dashboardRequest = data?.dashboardRequest;
  ...
  roleType: any;
  sourceName: any = null;
  dashboardRequest: any;
  comparisonData: any; // Comparison-specific data (latestDateOfReport, selectedDateOfReport, calculationOn, etc.)
  ...
  isFormValid(): boolean {
    ...
    const dateValid = !this.isEndDateInvalid();
  ...
  onSubmit() {
    ...
    const data: any = {
      ...
      sourceName: this.sourceName ? this.constants.SourceNameForCampaign.PR : this.constants.SourceNameForCampaign.ETB,
      dashboardRequest: this.dashboardRequest,
      // Comparison-specific data for PR source
    ...
    if (this.sourceName !== this.constants.SourceNameForCampaign.PR) {
      ...
      this.msmeService.prCampaignDetailsSave(data, !this.inEditMode).subscribe((response: any) => {
        this.isSubmitButtonDisabled = false;
  ...
  formatFilteredData(filteredList: any[]): string {
    ...
    filteredList.forEach(item => {
      ...
      // Skip empty, null, undefined, or dot-only values
      if (val === null || val === undefined) return;
      val = String(val);
      if (val === '•' || val === '●' || val.trim() === '' || val.trim() === '()') return;

      // Remove empty parentheses from value
      val = val.replace(/\s*\(\s*\)\s*/g, '').trim();
      
      // Skip if value becomes empty after cleanup
    ...
    return Object.keys(grouped)
      ...
      .map(key => {
        const values = grouped[key].filter((v: string) => v && v !== '•' && v !== '●' && v.trim() !== '');
