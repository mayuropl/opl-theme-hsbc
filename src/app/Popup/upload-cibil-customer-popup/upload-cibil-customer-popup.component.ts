...
import {CibilDownloadUrls, CibilUploadTypes, Constants, CrifDownloadUrls} from '../../CommoUtils/constants';
...
@Component({
  ...
  templateUrl: './upload-cibil-customer-popup.component.html',
  styleUrl: './upload-cibil-customer-popup.component.scss'
})
export class UploadCibilCustomerPopupComponent  implements OnInit {
  ...
  constructor(public dialog: MatDialog, private modalService: NgbModal, private msmeService: MsmeService, private commonService: CommonService,
    ...
    if (dialogData && dialogData.isCrif) {
      this.isCrif = true;
    }

    this.sharedService.getCibilCustomerUploadStatusClickEvent().subscribe((message) => {
  ...
  fetchBulkUploadHistory(page?, onPageChangeFlag?: boolean, approvalStatus?: String): void {
    ...
    const serviceCall = this.isCrif
      ? this.msmeService.getCrifCustomerUploadHistory(filterJson)
      : this.msmeService.getCommCibilUploadedFileData(filterJson);
  ...
  submit() {
    ...
    const serviceCall = this.isCrif
      ? this.msmeService.crifUploadCibilFileFromFileName(req)
      : this.msmeService.commercialCibilBulkExcelUpload(req);
  ...
  getData(type, url, fileId: any ,name?:any) {
    ...
    if (this.isCrif) {
      createMasterJson.url = '/download_customer_uploaded_file?file_id=' + fileId + '&type=' + type;
    } else {
    ...
    const serviceCall = this.isCrif
      ? this.msmeService.getCrifDataByFileId(createMasterJson)
      : this.msmeService.getCommCibilUploadedDataByFileId(createMasterJson);
  ...
  protected readonly CrifDownloadUrls = CrifDownloadUrls;
}
