...
import { CustomizeColumnsstructuredPopupComponent } from 'src/app/Popup/customize-columns-structured-popup/customize-columns-popup.component';

@Component({
  selector: 'app-product-level-chart-structured',
  templateUrl: './product-level-chart.component.html',
  styleUrl: './product-level-chart.component.scss'
})
export class ProductLevelChartStructuredComponent {
  showChart: boolean = true;
  optionList = ['Product Level', 'Customer Level'];
  mySelectedValue: string = 'Product Level';
  ...
  pageData: any;
  constructor(private chartConfigService: ChartConfigService, public dialog: MatDialog,private decimalPipe: DecimalPipe , protected commonService: CommonService) {
  ...
  ngOnChanges(changes: SimpleChanges) {
    console.log('product level opp data: ', this.productLevelOppData);
...
getHsbcLabel(displayName: string): string {
  ...
  setupcustomerlevelChartOptions(customerLevelChartData : any){
    ...
    const onlyCompetition = this.customerLevelChartData.find(
    (d: any) => d.display_name === "Only Competition (0%)"
  );
  const sole = this.customerLevelChartData.find(
    (d: any) => d.display_name === "Sole (100%)"
  );

    const midRangeItems  = this.customerLevelChartData.filter((d) =>
    lowToDominant.includes(d.display_name)
   );

    const midRangeTotalCustomers = midRangeItems.reduce(
    (sum, d) => sum + (d.unique_cust_count || 0),
    0
  );
  const midRangeTotalValue = midRangeItems.reduce(
    (sum, d) => sum + (parseFloat(d.total_utilization) || 0),
    0
  );

  // Determine which slice should be selected and sliced
    let selectedSliceName: string | null = null;
    if (this.selectedLegendItem === 'Sole (100%)') {
      selectedSliceName = 'Sole';
    } else if (this.selectedLegendItem === 'Only Competition (0%)') {
      selectedSliceName = 'Competition';
    } else if (this.selectedLegendItem === 'Multiple') {
      selectedSliceName = 'Multiple';
    }

  const chartData = [
        {
      name: "Competition",
      y: onlyCompetition?.unique_cust_count || 0,
      value: onlyCompetition?.total_utilization || 0,
      color: this.getPieGradientColor(0),
      selected: selectedSliceName === 'Competition',
      sliced: selectedSliceName === 'Competition',
    },
    {
      name: "Multiple",
      y: midRangeTotalCustomers,
      value: midRangeTotalValue,
      color: this.getPieGradientColor(1),
      selected: selectedSliceName === 'Multiple',
      sliced: selectedSliceName === 'Multiple',
    },
    {
      name: "Sole",
      y: sole?.unique_cust_count || 0,
      value: sole?.total_utilization || 0,
      color: this.getPieGradientColor(2),
      selected: selectedSliceName === 'Sole',
      sliced: selectedSliceName === 'Sole',
    },
  ];
    ...
    this.customerlevelChartOptions = {
      ...
      chart: {
        ...
        custom: {
          ...
          selectedPoint: this.selectedLegendItem ? (this.selectedLegendItem === 'Sole (100%)' ? 'Sole' : this.selectedLegendItem === 'Only Competition (0%)' ? 'Competition' : this.selectedLegendItem === 'Multiple' ? 'Multiple' : null) : null,
        },
        events: {
          render() {
            const chart = this,
          ...
          events: {
            click: (function(component) {
              return function() {
                ...
                // Always select the clicked series (single click behavior)
                // Close previous selection and highlight new one
                if (pointName === 'Sole') {
                  component.selectedLegendItem = 'Sole (100%)';
                  component.selectedOtherItems.clear();
                } else if (pointName === 'Competition') {
                  component.selectedLegendItem = 'Only Competition (0%)';
                  component.selectedOtherItems.clear();
                } else if (pointName === 'Multiple') {
                  component.selectedLegendItem = 'Multiple';
                  const lowToDominant = [
                    "Low (>0 % - 25%)",
                    "Competitive (25.01% - 50%)",
                    "Strong (50.01% - 75%)",
                  ...
                  component.selectedOtherItems.clear();
                  component.customerLevelChartData.forEach((dataItem: any) => {
                    if (lowToDominant.includes(dataItem.display_name)) {
                      component.selectedOtherItems.add(dataItem.display_name);
                    }
                  });
                }

                // Update chart to reflect selection
  ...
  onLegendItemClick(item: any) {
     let chartSegmentName: string | null = null;
    const lowToDominant = [
      "Low (>0 % - 25%)",
      "Competitive (25.01% - 50%)",
      "Strong (50.01% - 75%)",
    ...
    const isOtherItem = lowToDominant.includes(item.display_name);

    if (item.display_name === 'Only Competition (0%)') {
      chartSegmentName = 'Competition';
       if (this.selectedLegendItem === item.display_name) {
        this.selectedLegendItem = null;
        chartSegmentName = null;
      } else {
        this.selectedLegendItem = item.display_name;
        this.selectedOtherItems.clear();
      }
    } else if (item.display_name === 'Sole (100%)') {
      chartSegmentName = 'Sole';
       if (this.selectedLegendItem === item.display_name) {
        this.selectedLegendItem = null;
        chartSegmentName = null;
      } else {
       ...
       chartSegmentName = 'Multiple';
      if (this.selectedOtherItems.has(item.display_name)) {
         this.selectedOtherItems.clear();
        this.selectedLegendItem = null;
        chartSegmentName = null;
      } else {
  ...
  isLegendItemActive(item: any): boolean {
    if (item.display_name === 'Only Competition (0%)' || item.display_name === 'Sole (100%)') {
      return this.selectedLegendItem === item.display_name;
    } else {
      // For "other" items, check if any of them are selected
      return this.selectedOtherItems.has(item.display_name);
    }
  }
  ...
  CustomizeColumn_popup() {
    if (this.mySelectedValue == 'Product Level') {
      const dialogRef = this.dialog.open(CustomizeColumnsstructuredPopupComponent, {
  ...
  onChnageOrder(columnName: string, apiType: string, customerName: string = null) {
    ...
    const sortSearchReqParent = { ...this.parentTableSortOrderObj, ...parentSearchReq };
    const sortSearchReqChild = { ...this.childTableSortOrderObj, ...childSearchReq };

    switch (apiType) {
      case 'productWiseCustomerData':
        this.productWiseCustomerData = null;
        this.isLoading = true;
        this.pageChangeProductLevel.emit({ productId: this.productId, pageSize: this.ppageSize, offset, ...sortSearchReqChild });
        break;

      case 'productLevelOppData':
        this.productLevelOppData = null;
        this.isLoading = true;
        this.pageChangeProductParent.emit({ pageSize: this.productPageSize, offset, ...sortSearchReqParent });
        break;

      case 'customerWiseProductsData':
        this.customerWiseProductsData = null;
        this.isLoading = true;
        this.pageChangeCustomerProducts.emit({ customerName, pageSize: this.customerProductPageSize, offset, ...sortSearchReqChild });
        break;

      case 'customerLevelOppData':
        this.customerLevelOppData = null;
        this.isLoading = true;
        this.pageChangeCustomerLevel.emit({ pageSize: this.pageSize, offset, ...sortSearchReqParent });
        break;
...
emitParentSearch() {
  ...
  if (this.mySelectedValue === 'Product Level') {
    this.productLevelOppData = null;
    this.isLoading = true;
    this.pageChangeProductParent.emit({ pageSize: this.productPageSize, offset, ...this.parentTableSortOrderObj });
  } else {
    this.customerLevelOppData = null;
    this.isLoading = true;
    this.pageChangeCustomerLevel.emit({ pageSize: this.pageSize, offset, ...this.parentTableSortOrderObj });
  }
}
...
emitChildSearch(customerName: string = null) {
  ...
  if (this.mySelectedValue === 'Product Level') {
    this.productWiseCustomerData = null;
    this.isLoading = true;
    this.pageChangeProductLevel.emit({ productId: this.productId, pageSize: this.ppageSize, offset, ...this.childTableSortOrderObj });
  } else {
    this.customerWiseProductsData = null;
    this.isLoading = true;
    this.pageChangeCustomerProducts.emit({ customerName, pageSize: this.customerProductPageSize, offset, ...this.childTableSortOrderObj });
...
excelDownloadProductLevel(){
  this.emitExcelDownloadProductLevel.emit({prodOrCustlevel : this.mySelectedValue,prodExcelColumn : this.allColumns,  custExcelColumn : this.allColumnsCustExcel});
}
...
onPageChange(pageNumber: number) {
  ...
  const sortSearchReq = { ...this.parentTableSortOrderObj, ...searchReq };
...
onProductPageChange(pageNumber: number) {
  ...
  const searchReq = this.prepareSearchRequest(this.productLevelChildSearchFilters);
  const sortSearchReq = { ...this.childTableSortOrderObj, ...searchReq };
