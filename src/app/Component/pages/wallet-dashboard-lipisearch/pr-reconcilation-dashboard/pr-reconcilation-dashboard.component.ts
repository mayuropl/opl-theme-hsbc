...
export class PrReconcilationDashboardComponent {
  ...
  ngOnInit(): void {
    ...
    this.getCommercialReconsilationData();
  }
  ...
  getAlertsSubTabData(_index: number): void {
    ...
    getCommercialReconsilationData(): void {
      this.isLoading= true;
      const payload: any = {
        "fileId": this.fileId,
        "withInSixMonth": !this.selectedComparisonType,
  ...
  downloadOptions = [
    ...
    { label: 'Product Level With In Six Month', level: 'Product Level', withInSixMonth: true },
    { label: 'Customer Level Total', level: 'Customer Level', withInSixMonth: false },
    { label: 'Customer Level With In Six Month', level: 'Customer Level', withInSixMonth: true }
  ];
