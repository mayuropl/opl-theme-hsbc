     ...
     if (showSummaryState === "true") {
       ...
       if (savedSummaryFiltersStr) {
         try {
           ...
           const searchByKeys = ['city_id', 'segment_id', 'rm_id', 'employee_id', 'company_name'];
...
cleanJsonString(str: string): string {
  ...
  getGlobalFilters() {
    ...
    const req = {
      queryKey: 'dropdown_filters'
    }
    this.msmeService.getPRDashboardFilterLipisearch(req).subscribe(
      (response) => {
    ...
    const req = {
      ...
      queryKey: "lending_analysis",
    }

    this.filtersffNavigateFinale = { ...this.filtersff };

    this.msmeService.getPrDashboardLendingAnalysisDataLipiSearch(req).subscribe(
      (response) => {
  ...
  getTrendsLendingCharts() {
    ...
    const req = {
      ...
      dashboard_id: 2,
      filters : this.filtersff,
      queryKey: "lending_analysis_trend",
    }
    ...
    const req = {
      ...
      dashboard_id: 2,
      filters : this.filtersff,
      queryKey : "segment_level_analysis"
    }
  ...
  setAllFilters(childFilters: any) {
    ...
    const searchByKeys = ['city_id', 'segment_id', 'rm_id', 'employee_id', 'company_name', 'which_level'];
...
onFetchProductLevelCustomers(productLevelReq: any) {
    ...
    this.msmeService.getPrDashboardLendingAnalysisDataLipiSearch(req).subscribe((response) => {
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      if (result) {
        ...
        if (result.filterState) {
          ...
          // Apply filter - refresh data based on selected hierarchy nodes
