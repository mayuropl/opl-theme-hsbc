...
onSearchValueChange(value: string) {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['restoredFilters'] && hasRestoredSummaryFilters) {
      ...
      const searchByFilterKeys = ['city', 'city_id', 'segment', 'segment_id', 'rm', 'rm_id', 'company_name', 'city_name', 'seg_name', 'employee_name', 'employee_id', 'company_name_search'];
    ...
    if (changes['filters']) {
      ...
      setTimeout(() => {
        ...
        if (!this.skipSummaryApiCall && !this.isApplyButtonClicked) {
          this.applyFilters();
        }
      });
    }
...
private normalize(text: string): string {
  ...
  isProductTypeGroupMatch(productTypeLabel: string, groupName: string): boolean {
    ...
    for (const [productType, groupNames] of Object.entries(customMappings)) {
      if (productTypeLabel.includes(productType)) {
        return groupNames.some(mappedGroupName => groupName.includes(mappedGroupName));
      }
    }

    // Fallback: check if label contains groupName or groupName contains label
    if (productTypeLabel.includes(groupName) || groupName.includes(productTypeLabel)) {
      return true;
    }
...
onDropdownOpened() {
  ...
  switch (filterName) {
    ...
    case 'product_sub_type': return 'fi fi-rr-layers blue_text mr-1';
