...
@Component({
  ...
  templateUrl: './rmbank-statement-analysis.component.html',
  styleUrl: './rmbank-statement-analysis.component.scss'
})
export class RMBankStatementAnalysisComponent implements OnInit, OnDestroy {
  today: string;
  panForm: FormGroup;
  ...
  getFilteredFormValue() {
    ...
    filteredFormValue["version"] = Number(1);
  ...
  onRefresh() {
    ...
    createMasterJson.version = Number(1);
    this.msmeService.createBsMaster(createMasterJson)
      .subscribe((response: any) => {
        ...
        if (response.status === 200) {
          ...
          this.router.navigate(['/hsbc/upload-bank-statement', this.commonService.toBTOA(this.getControlValue('pan'))], { state: { data: this.pageData }} );
  ...
  navigateToView(bsHistory: BankStatementHistory) {
    ...
    this.router.navigate([`/hsbc/rmBankStatementAnalysisView/${this.commonService.toBTOA(bsHistory.bsId + '')}`], { state: { routerData,data:this.pageData } });
  }
