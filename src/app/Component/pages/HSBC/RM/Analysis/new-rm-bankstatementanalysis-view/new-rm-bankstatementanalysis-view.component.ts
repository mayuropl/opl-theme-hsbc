...
@Component({
  ...
  templateUrl: './new-rm-bankstatementanalysis-view.component.html',
  styleUrl: './new-rm-bankstatementanalysis-view.component.scss'
})
export class NewRmBankstatementanalysisViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.pageData = history.state.data;
      ...
      this.pageData  = this.commonService.getPageData(this.consValue.pageMaster.ANALYTICS,this.consValue.pageMaster.FINANCIALS_ANALYSIS,this.consValue.pageMaster.BANK_STATEMENT_ANALYSIS)
    }
    console.log("this.pageData",this.pageData);
    this.consolidatedRequired = this.commonService.getStorage('consolidatedRequired',true);
    if(this.consolidatedRequired === 'true'){
      this.consolidatedSelected = true;
    }
    resetGlobalHeaders();
  ...
  getData() {
    ...
    this.msmeService.multiBankStatementGetData(json).subscribe((response: any) => {
  ...
  downloadExcelFile(fileType?:any) {
    ...
    this.msmeService.multiDownloadBSAnalysisReport(req).subscribe(res => {
      ...
      if (res.status === 200 && res.fileBytes) {
        //  this.downloadExcel(res.contentInBytes, 'BS Analysis_'+this.bsId);
        this.downloadZip(res.fileBytes, 'BS Analysis_' + this.bsId,fileType);
  ...
  downloadZip(byteData: any, fileName: string , fileType) {
