...
@Component({
  selector: 'app-custom-daterangepanel', 
  templateUrl: './custom-daterangepanel.component.html',
  styleUrl: './custom-daterangepanel.component.scss'
})
export class CustomDaterangepanelComponent<D> {
  ...
  private calculateDateRange(rangeName: CustomPreset): [start: D, end: D] {
    ...
    switch (rangeName) {
      ...
      case 'last 60 days': {
          const start = this.dateAdapter.addCalendarDays(today, -59);
          return [start, today];
      }
      case 'last 90 days': {
        const start = this.dateAdapter.addCalendarDays(today, -89);
        return [start, today];
      }
      case 'All': {
