...
} from '../../../../Popup/upload-cibil-consumer-customer-popup/upload-cibil-consumer-customer-popup.component';

@Component({
  selector: 'app-consumer-cibil-bulk-upload',
  templateUrl: './consumer-cibil-bulk-upload.component.html',
  styleUrl: './consumer-cibil-bulk-upload.component.scss'
})
export class ConsumerCibilBulkUploadComponent implements OnInit{
  ...
  constructor(public dialog: MatDialog, private modalService: NgbModal, private msmeService: MsmeService, private commonService: CommonService,
    ...
    this.sharedService.getConsumerCibilUploadStatusClickEvent().subscribe((message)=>{
  ...
  ngOnInit(): void {
    ...
    if(!this.pageData || this.pageData === 'undefined'){
      this.pageData  = this.commonService.getPageData(Constants.pageMaster.BULK_UPLOAD,Constants.pageMaster.CONSUMER_CIBIL)
    }
  ...
  createBulkUploadForm(bulkUploadDetails) {
    this.commCibilUploadDetailsForm = this.formBuilder.group({
      fileUpload: new UntypedFormControl(bulkUploadDetails?.fileUpload ? bulkUploadDetails.fileUpload : '', []),
  ...
  // }
  callGetMAsterId(){
    this.msmeService.getConsumerFileMasterList().subscribe((res: any) => {
      if (res.status === 200) {
  ...
  callMarkCompleted(file:any){
    ...
    this.msmeService.callConsumerMarkFileCompleted(filterJson).subscribe((res: any) => {
      if (res.status === 200) {
        console.log('Response === > ', res);
        let response = JSON.parse(res?.data);
        this.fileDropdown = response?.listData;
  ...
  UploadCustomer_popup(): void {
    const dialogRef = this.dialog.open(UploadCibilConsumerCustomerPopupComponent, {
      data: this,
