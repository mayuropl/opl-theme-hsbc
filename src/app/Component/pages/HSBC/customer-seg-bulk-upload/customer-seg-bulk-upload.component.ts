...
@Component({
  selector: 'app-customer-seg-bulk-upload',
  templateUrl: './customer-seg-bulk-upload.component.html',
  styleUrl: './customer-seg-bulk-upload.component.scss'
})
export class CustomerSegBulkUploadComponent {
  ...
  custSegementBulkUploadHistory: any;
...
protected readonly consValue = Constants;
  ngOnInit(): void {
    ...
    if(!this.pageData || this.pageData === 'undefined'){
      this.pageData  = this.commonService.getPageData(this.consValue.pageMaster.BULK_UPLOAD,this.consValue.pageMaster.CUSTOMER_SEG_MASTER)
    }
    resetGlobalHeaders();
    GlobalHeaders['x-path-url'] = '/hsbc/customer_seg_bulk_upload';
  ...
  fetchBulkUploadHistory(page?, onPageChangeFlag?: boolean, approvalStatus?: String): void {
    ...
    data.tableType = 'CUS-SEGMENT'
    // this.data.tab = tabId;

    this.msmeService.getCustomerSegementFileData(data).subscribe((res: any) => {
      if (res && res.status == 200) {
        ...
        this.custSegementBulkUploadHistory = res.listData;
  ...
  submit() {
    ...
    this.msmeService.uploadCustomerSegement(formData,this.userName).subscribe(res => {
      console.log("res=========",res);
      if (res.status === 200) {
  ...
  downloadTemplate() {
    ...
    const fileUrl = 'assets\files\Customer_Segement_Bulk_Upload.xlsx';  // Path to the file in the assets folder
    this.downloadFile(fileUrl).subscribe(blob => {
      const a = document.createElement('a');
      const objectUrl = URL.createObjectURL(blob);
      a.href = objectUrl;
      a.download = 'Customer_Segement_Bulk_Upload.xlsx';  // Update with the actual file name and extension
      a.click();
  ...
  getData(type, batchId) {
    ...
    createMasterJson.tableType = 'CUS-SEGMENT';
    ...
    if (type == 1) {
      ...
      this.msmeService.getCustomerSegDataByMstId(createMasterJson).subscribe((res: any) => {
        if (res.status == 200) {
      ...
      ////console.log("res=========",res);
      this.commonService.successSnackBar("Your file is being prepared. The download will begin shortly…");
      this.msmeService.getCustomerSegDataByMstId(createMasterJson).subscribe(res => {
        if (res.status == 200) {
          this.downloadDataInExcel(res.data, 2, type);
        }
      }, error => {
  ...
  downloadDataInExcel(excelData:any, type, reqType) {
    ...
    if (reqType == 1) {
      ...
      excelData.forEach((element, i) => {
        ...
        allApplications = [{
          Sr_no: index,
          'Segement code': element.seg_code ? element.seg_code : '-',
          'Segement name': element.seg_name ? element.seg_name : '-',
          'Business': element.business ? element.business : '-',
          'Opreation':element.operation ?element.operation:'-',
      ...
      excelData.forEach((element, i) => {
        ...
        allApplications = [{
          ...
          'Failure Reason': element.failure_reason ? element.failure_reason : '-',
...
getUserDetails() {
  ...
  downloadAllCustomerSegement(){

    this.msmeService.downLoadAllCustomerSegement().subscribe(res=>{
      this.downloadExcel(res.file,"ALL_Customer_Segement")
    });
