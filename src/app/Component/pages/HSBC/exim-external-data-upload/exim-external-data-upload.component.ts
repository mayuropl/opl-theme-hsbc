...
@Component({
  selector: 'app-exim-external-data-upload',
  templateUrl: './exim-external-data-upload.component.html',
  styleUrl: './exim-external-data-upload.component.scss'
})
export class EximExternalDataUploadComponent implements OnInit {
  ...
  constructor(private fb: UntypedFormBuilder, private commonService: CommonService, private msmeService: MsmeService, private http: HttpClient, private sharedService: SharedService) {
    this.sharedService.getEximUploadFileStatusChangeEvent().subscribe((message)=>{
  ...
  ngOnInit(): void {
    // Listen for changes in vendorName or fileType
    this.uploadForm.get('vendorName')?.valueChanges.subscribe(() => this.toggleFileUploadControl());
    this.uploadForm.get('fileType')?.valueChanges.subscribe(() => this.toggleFileUploadControl());

    // Initial toggle
    this.toggleFileUploadControl();
  ...
  initForm() {
    this.uploadForm = this.fb.group({
        vendorName: ['panjiva', Validators.required],
  ...
  getAcceptedFileTypes(): string {
    const vendorName = this.uploadForm.get('vendorName')?.value;
    return vendorName === 'panjiva' ? '.csv' : '.xlsx';
  }

  getFileTypeText(): string {
    const vendorName = this.uploadForm.get('vendorName')?.value;
    return vendorName === 'panjiva' ? 'CSV' : 'Excel';
  }

  canUploadFile(): boolean {
    const vendorName = this.uploadForm.get('vendorName')?.value;
    const fileType = this.uploadForm.get('fileType')?.value;
    return !!(vendorName && fileType);
  }
  ...
  onSubmit() {
    if (this.uploadForm.valid) {
      ...
      this.msmeService.eximDashboardUpload(req).subscribe(res => {
        if (res.status === 200) {
  ...
  retryUpload(uuid) {
    ...
    req.uuid = uuid;
    console.log("req ::::::::::::: ", req);
    this.msmeService.eximDashboardRetryUpload(req).subscribe(res => {
      if (res.status === 200) {
        this.commonService.successSnackBar(res.message);
      } else {
        this.commonService.warningSnackBar(res.message);
  ...
  getAudit() {
    ...
    this.msmeService.eximDashboardAudit(page, pageSize).subscribe(res => {
      if (res.status === 200) {
  ...
  openFileDialog(fileInput: HTMLInputElement) {
    if (!this.canUploadFile()) {
      this.commonService.warningSnackBar("Please select Vendor and File Type first.");
      return;
    }
    setTimeout(() => {
      fileInput.click();
  ...
  downloadTemplate(fileType) {
    ...
    let fileName;
    if(fileType == 1) {
      if(this.uploadForm.value.vendorName == 'panjiva') {
        fileUrl = 'assets/files/EximDashboard/Panjiva Export Template.csv';
        fileName = 'Panjiva Export Template.csv';
      } else {
        fileUrl = 'assets/files/EximDashboard/Volza Export Template.xlsx';
        fileName = 'Volza Export Template.xlsx';
      }
    } else {
      if(this.uploadForm.value.vendorName == 'panjiva') {
        fileUrl = 'assets/files/EximDashboard/Panjiva Import Template.csv';
        fileName = 'Panjiva Import Template.csv';
      } else {
        fileUrl = 'assets/files/EximDashboard/Volza Import Template.xlsx';
        fileName = 'Volza Import Template.xlsx';
      }
    }
  ...
  downloadFile(actualFileName, filePath:String){
    ...
    this.msmeService.eximDashboardDownloadFile(req).subscribe(
      (response: any) => {
  ...
  downloadByBucketRef(type, actualFileName :string, filePath:string) {
    ...
    this.msmeService
      .eximDashboardDownloadFile(formData)
      .subscribe(
        (resp) => {
          ...
          const a = document.createElement('a');
          a.href = objectUrl;
          a.download = actualFileName.split(".")[0]+".csv";
          a.click();
          URL.revokeObjectURL(objectUrl);
