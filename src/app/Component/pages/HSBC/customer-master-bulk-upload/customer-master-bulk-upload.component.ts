...
@Component({
  selector: 'app-customer-master-bulk-upload',
  templateUrl: './customer-master-bulk-upload.component.html',
  styleUrls: ['./customer-master-bulk-upload.component.scss']
})
export class CustomerMasterBulkUploadComponent implements OnInit {
...
protected readonly consValue = Constants;
  ngOnInit(): void {
    ...
    console.log('history data',history.state,'history page data: ', history.state.data);

    if(!this.pageData || this.pageData === undefined || this.pageData === null){
      ...
      console.log('page Dataaaa: ', this.pageData);
    }
    
    let subpageData = this.pageData.subSubpages.filter(data => data.subpageId === this.consValue.pageMaster.EXIM_INTERNAL_DATA);
    ...
    resetGlobalHeaders();
    GlobalHeaders['x-path-url'] = '/hsbc/Customer-Bulk-Upload';
  ...
  createBulkUploadForm(bulkUploadDetails) {
    this.bulkUploadDetailsForm = this.formBuilder.group({
      exchangeRateYTD: new UntypedFormControl( '0', [Validators.required]), // Set to '0' for EXIM Internal Data Upload
    ...
    // Set default values for EXIM Internal Data Upload
    this.fileType = "Exim Internal Data Upload";
    this.showDropdown = true;
  }
  /**
  ...
  //   this.commonService.warningSnackBar('Please upload Export file');
  //   return false;
  // }
  // if(this.bulkUploadDetailsForm.get('exchangeRateYTD')?.value === '0' && this.selectedModuleId === 2){
  //   this.commonService.warningSnackBar('Please upload Import file');
  //   return false;
  // }
    ...
    if (this.bulkUploadDetailsForm.get('exchangeRateYTD')?.value === '0' && this.selectedModuleId === 1){
      ...
      this.msmeService.eximBulkUploadDataForExport(formData, this.userName).subscribe(res=>{
        ...
        if(res.status == 200){
          this.fileType=null;
          // this.bulkUploadDetailsForm.get('fileUpload')?.setValue('');
